        ...
        struct light_state_t const* state)
{
    ...
    char lightson[PROP_VALUE_MAX];
    struct color *nearest = NULL;
    property_set("lights.notification", "off");
    system("pkill lightsd");
    if(!dev) {
        return -1;
    }
    ...
    if (blink) {
        ...
        // Make sure the values are between 0.5 and 7 seconds
        if (onMS < 500)
            onMS = 500;
        else if (onMS > 7000)
            onMS = 7000;
        ...
        if (offMS < 500)
            ...
            offMS = 7000;
    //Call custom lights daemon to set blinking.     
    int ret;
    char buff[50];
    sprintf(buff, "lightsd %d %d %d %d %d", red, green, blue, onMS, offMS);
    ret = system(buff);
    return 0;
